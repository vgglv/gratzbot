...
async def buy_farm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_correct_chat(update):
        return
    user = app.db.get_user(str(update.effective_user.id), update.effective_user.first_name)
    if user.gold < constants.FARM_PRICE:
        response = f'<b>{user.name}</b> не хватает золота. Требуется {constants.FARM_PRICE} золотых для покупки фермы.'
        await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")
        return
    user.set_farm(user.farm + 1)
    user.set_gold(user.gold - constants.FARM_PRICE)
...
* <b>/buy_farm</b> - СТОИМОСТЬ: {constants.FARM_PRICE} зол. 
...
* <b>/attack</b> - СТОИМОСТЬ: {constants.ATTACK_COST}. ОПИСАНИЕ: совершить набег на того, кому реплаим. ШАНС успеха = {constants.ATTACK_SUCCESS_RATE}%. При успешном набеге у оппонента отжимается 1 ферма и передается вам, в противном случае, ваша ферма отдается оппоненту. Нельзя использовать против тех, у кого только 1 ферма.

* <b>/steal</b> - СТОИМОСТЬ: 0, ОПИСАНИЕ: украсть золото у того, кому реплаим. ШАНС успеха = {constants.STEAL_SUCCESS_RATE}%. При провале у вас отнимается золото и отдается тому, у кого вы пытались украсть. При успешной краже, вы крадете 1 золото.

* <b>/casino N</b> - поставить N золотых на казино, шанс удвоить их равен {constants.CASINO_SUCCESS_RATE}%, в противном случае ваше золото уходит в <b>призовой фонд лотереи</b>.

* <b>/lottery</b> - СТОИМОСТЬ: {constants.LOTTERY_COST}. ШАНС успеха: {constants.LOTTERY_SUCCESS_RATE}%. При выигрыше вы получаете <b>призовой фонд лотереи</b>.
...
async def steal(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_same_user(sending_user, receiving_user):
        return
    if sending_user.gold < constants.STEAL_COST:
        response = f'<b>{sending_user.name}</b>, нужно больше золота ({constants.STEAL_COST}). У вас: {sending_user.gold}.'
        await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")
        return
    ...
    response = f'Готовится кража... \n{sending_user.name} попытается украсть у {receiving_user.name} золото (шанс успеха {constants.STEAL_SUCCESS_RATE}%)'
    await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")

    if app.utils.roll_for_success(constants.STEAL_SUCCESS_RATE):
...
async def collect_farm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    days = delta_time // constants.ONE_DAY_IN_SECONDS
    leftover = delta_time - (days * constants.ONE_DAY_IN_SECONDS)
    if days <= 0:
        next_time_str = time.strftime('%H:%M:%S', time.gmtime(constants.ONE_DAY_IN_SECONDS - delta_time))
    ...
    next_time = user.saved_date + constants.ONE_DAY_IN_SECONDS - current_time
...
async def attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_same_user(sending_user, receiving_user):
        return
    if sending_user.gold < constants.ATTACK_COST:
        response = f'<b>{sending_user.name}</b>, нужно больше золота ({constants.ATTACK_COST}), у вас {sending_user.gold}.'
        await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")
        return
    ...
    sending_user.set_gold(sending_user.gold - constants.ATTACK_COST)
    response = f'Готовим набег...\n{sending_user.name} атакует {receiving_user.name}.\nШанс успеха {constants.ATTACK_SUCCESS_RATE}%...\nНа подготовку набега у {sending_user.name} потратилось {constants.ATTACK_COST} золота\nИтого у него осталось: {sending_user.gold} {app.utils.declensed_gold(sending_user.gold)}.'
    await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")

    if app.utils.roll_for_success(constants.ATTACK_SUCCESS_RATE):
...
async def gratz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    response = f"<b>{receiving_user.name}</b> грац! Получите +1 зол.\n\n{get_stats_gold(sending_user)}\n\n{get_stats_gold(receiving_user)}"
    await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")
...
async def casino(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        response = f"<b>{sending_user.name}</b>, неправильный ввод команды, RTFM. /faq"
        await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")
        return
    ...
    if app.utils.roll_for_success(constants.CASINO_SUCCESS_RATE):
        big_money = gold_am * 2
...
async def lottery(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if sending_user.gold < constants.LOTTERY_COST:
        response = f"<b>{sending_user.name}</b> у вас нет денег на покупку лотерейного билета."
        await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")
        return

    sending_user.decrement_gold()
    response = f"{sending_user.name} купил лотерейный билет... шанс выигрыша составляет {constants.LOTTERY_SUCCESS_RATE}%, призовой фонд: {gold_in_bank} {app.utils.declensed_gold(gold_in_bank)}!"
    await context.bot.send_message(chat_id=update.effective_chat.id, text=response, parse_mode="HTML")

    if app.utils.roll_for_success(constants.LOTTERY_SUCCESS_RATE):
